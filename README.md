...
| OA 원문 탐색 | Unpaywall·CORE 교차 조회로 합법적 공개본 확보 | §6.1-7 |
| 주제별 지식저장소 | 파일은 주제 폴더에 **한 번만** 저장, 일자 이력은 Manifest 로 관리 | §9 |
| 다단계 중복 제거 | DOI → 공식 ID → 파일 SHA256 → 텍스트 SHA256 → 제목 fuzzy | §11 |
| 공유 API 효율화 | 같은 호스트를 여러 소스가 쓸 때 rate limit·robots·동일 요청을 실행 단위로 묶음 | §18.1-2 |
...
- [docs/시스템_소개.md](docs/시스템_소개.md) — **핵심 기능 요약 (공유용)**
